...
export class SchemaEngine {
     ...
     */
    public async checkDatabaseExistence(target: TargetDb): Promise<QueuedQuery | null> {
        try {
            const adminPool = target.pg.getAdminPool();
            const res = await adminPool.query('SELECT datname FROM pg_database WHERE datname = $1', [target.config.NAME]);
     ...
     */
    public async generateDiff(target: TargetDb, dropOrphans: boolean = false): Promise<QueuedQuery[]> {
        ...
        try {
            ...
            if (!this.options.mute) log.warn('Could not fetch existing tables (DB might not exist yet)');
        ...
        for (const schemaDir of databasePaths) {
            ...
            if (!this.options.mute) log.spin(`[${id}] Reading ${files.length} schema files...`);
                ...
                for (let [tableName, tableCols] of Object.entries(data)) {
                    ...
                    if (tablesReal.includes(tableName)) {
                        ...
                        try {
                            ...
                            log.error(`Error analyzing table ${tableName}: ${(err as Error).message}`);
                        }
                    } else {
                        // CREATE
        ...
        if (dropOrphans) {
            for (const existingTable of tablesReal) {
             ...
             if (orphans.length > 0 && !this.options.mute) {
                 log.warn(`[${id}] ${orphans.length} orphan table(s) found. Use --drop-orphans to clean.`);
             }
        }
